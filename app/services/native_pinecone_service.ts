...
export default class NativePineconeService {
  ...
  async query(
    ...
    try {
      ...
      console.log(`✅ Found ${results.length} similar vectors (filter: ${JSON.stringify(filter || {})})`)
      return results
    } catch (error) {
   ...
   */
  async updateMetadataByIds(vectorIds: string[], metadataUpdates: Record<string, any>): Promise<void> {
    try {
      ...
      for (let i = 0; i < vectorIds.length; i += batchSize) {
        ...
        try {
          ...
          if (upsertData.length > 0) {
            ...
            console.log(`✅ Updated batch ${Math.floor(i / batchSize) + 1} (${upsertData.length} vectors)`)
          }
          
        } catch (error) {
          console.error(`⚠️ Failed to update batch ${Math.floor(i / batchSize) + 1}:`, error.message)
